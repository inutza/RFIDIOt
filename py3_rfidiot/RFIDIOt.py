...
class rfidiot:
		...
		if not self.NoInit:
			if self.readertype == self.READER_PCSC:
				...
				if readernum >= len(self.pcsc):
					print(f'There is no such reader #{readernum}, PCSC sees only {len(self.pcsc)} reader(s)')
					os._exit(True)
				try:
					...
					print(f'Could not create connection to {self.readername}')
					os._exit(True)
				...
				if (self.readername).find('OMNIKEY') == 0:
					...
					if self.readername.find('SDI010') == 0:
						...
						if self.readername.find('ACS ACR122U') == 0:
							self.readersubtype= self.READER_ACS
							self.pcsc_protocol= smartcard.scard.SCARD_PROTOCOL_T1
							self.hcard = None
						elif self.readername.find('ACS ACR1255U-J1') == 0:
							...
							self.readersubtype= self.READER_OMNIKEY
				if self.DEBUG:
					print(f'Reader Subtype: {self.readersubtype}')
				# create a connection
				try:
	...
	def info(self,caller):
		if len(caller) > 0:
			print(f'{caller} (using RFIDIOt v{self.VERSION})')
		if not self.NoInit:
			self.reset()
			self.version()
			if len(caller) > 0:
			...
			if self.readertype == self.READER_ACG:
				print(f'ACG {self.readername}', end=' ')
				print(f' (serial no: {self.id()})')
			if self.readertype == self.READER_FROSCH:
				print(f'Frosch {self.ToBinary(self.data[:16])}  / {self.ToBinary(self.data[16:32])}', end=' ')
				print(f' (serial no: {self.data[32:54]})')
			if self.readertype == self.READER_PCSC:
				print(f'PCSC {self.readername}')
	...
	def version(self):
		...
		if self.readertype == self.READER_FROSCH:
			if self.frosch(self.FR_RWD_Get_Version,''):
				...
				os._exit(True)
		if self.readertype == self.READER_ANDROID:
			print(f'Android version: {self.android.VERSION}')
	def id(self):
	...
	def settagtype(self,type):
		if self.readertype == self.READER_ACG:
			...
			self.errorcode= self.ser.readline()[:-2]
			if self.errorcode == 'O' + type.upper():
				self.tagtype= type
				return True
		if self.readertype == self.READER_FROSCH:
			if type == self.EM4x02:
	...
	def select(self, cardtype='A'):
		...
		if self.readertype == self.READER_PCSC:
			...
			try:
				...
				if self.readersubtype == self.READER_ACS:
					if self.DEBUG: print("acs detected select tag")
					self.acs_select_tag()
				else:
			...
			try:
				...
				if cardtype == 'A':
					...
					if result:
						...
						self.cid = result.cid
						if self.DEBUG:
							print(f'PUPI: {self.pupi}')
							print(f'ATR: {self.atr}')
							print(f'UID: {self.uid}')
							print(f'APPDATA: {self.appdata}')
					...
					if result:
						...
						self.uid = result.uid
						if self.DEBUG:
							print(f'SENSRES: {self.btsensres}')
							print(f'ID: {self.btid}')
						return True
					else:
						if self.DEBUG:
							print('Error selecting card')
					...
					if self.DEBUG:
						print(f'Error: Unknown card type specified: {cardtype}')
					return False
			except ValueError as e:
				self.errorcode = f'Error selecting card using LIBNFC: {e}'
		...
		if self.readertype == self.READER_ANDROID:
			try:
				...
				self.errorcode = f'Error reading card using Android: {e}'
		return False
	...
	def acs_send_apdu(self,apdu):
		...
		if myapdu[0] == 'd4':
			...
			if  myapdu[0] == 'ff' or myapdu[0] == '80':
				...
				if self.DEBUG: print("build pseudo command for 14a")
		...
		self.errorcode= f'{sw1:02X}{sw2:02X}'
		if self.errorcode == self.ISO_OK:
	...
	def acs_read_block(self,block):
		...
		if read:
			...
			self.errorcode= self.ISO_OK
			return True
		print(f"Can't read {self.ACS_TAG_TYPES[self.tagtype]} blocks")
		os._exit(True)
	...
	def acs_select_tag(self):
		...
		try:
			...
			print(f'unrecognised TAG type: {self.sel_res}')
			print(f'full ACS return: {self.data}')
	...
	def libnfc_mifare_login(self,block,key,keytype):
		...
		if keytype == 'BB':
			keytype= f'{pynfc.MC_AUTH_B:02x}'
		else:
		   keytype= f'{pynfc.MC_AUTH_A:02x}'
		loginblock= f'{block:02x}'
		#if self.tagtype == self.ACS_TAG_MIFARE_1K or self.tagtype == self.ACS_TAG_MIFARE_4K:
	...
	def libnfc_mifare_read_block(self, block):
		apdu= []
		apdu += f'{pynfc.MC_READ:02X}' # mifare read
		hexblock= f'{block:02x}'
	...
	def iso_7816_fail(self,code):
		...
		if code == self.ACG_FAIL:
			...
			os._exit(True)
		print(f"Failed - reason code {code} ( {self.ISO7816ErrorCodes[code]})")
		print()
		os._exit(True)
	def iso_7816_get_challenge(self,length):
        	...
        	le= f'{length:02x}'
        	if self.DEBUG:
                	print(f"DEBUG: requesting {length} byte challenge")
...
#			time.sleep(.1)
	...
	def send_apdu(self,option,pcb,cid,nad,cla,ins,p1,p2,lc,data,le):
		...
		dlength += len(le) / 2
		if self.DEBUG:
			print(f'sending: t{dlength:02x} {option} {command}')
...
#		return ret[4:int(le,16) * 2 + 4]
	...
	def login(self,sector,keytype,key):
		...
		if self.readertype == self.READER_ACG:
			...
			if not sector == '':
				if self.DEBUG:
					print(f'sending: l{sector:02x} {keytype} {key}')
			...
			if self.DEBUG:
				print(f'received: {self.errorcode}')
			if self.errorcode == 'L':
				self.errorcode= ''
				return True
			return False
		...
		if self.readertype == self.READER_PCSC:
			...
			if key not in self.PCSC_Keys:
				...
				if keytype == 'BB':
					...
					keynumoffset= 0
				apdu.append(f'{keynum + keynumoffset:02x}') # p2 - key number
				apdu.append(f'{len(key) / 2}:02x') # lc
	...
	def authenticate(self,sector,keytype, keynum):
			...
			if keytype == 'AA' or keytype == 'FF':
				...
				apdu.append(keytype)
			apdu.append(f'{keynum:02x}') # key number
	...
	def readblock(self,block):
		...
		if self.readertype == self.READER_LIBNFC:
			...
			return False
			## this doesn't make sense?! -- swapped the lines
			apdu= []
			apdu += self.PCSC_APDU['READ_BLOCK']
			apdu += f'{pynfc.MC_READ:02X}' # mifare read
			hexblock= f'{block:04x}'
			apdu.append(hexblock)
	...
	def writeblock(self,block,data):
		...
		if self.readertype == self.READER_ACG:
			self.ser.write(f'w{block:02x}{data}')
		...
		if self.readertype == self.READER_PCSC:
			...
			apdu.append(hexblock[0:2]) # p1
			apdu.append(hexblock[2:4]) # p2
			apdu.append(f'{(len(data) / 2):02x}') # le
			apdu.append(data)
			return self.pcsc_send_apdu(apdu)
	def writevalueblock(self,block,data):
		self.ser.write('wv{block:02x}{data}')
		x= self.ser.readline()[:-2]
		if x == data.upper():
			self.errorcode= ''
			return True
	...
	def GetRandom(self,size):
        	data= ''
        	for x in range(size):
                	data += f'{int(random.uniform(0,0xff)):02x}'
        	return data
	...
	def ToBinaryString(self,data):
		...
		for x in range(0,len(string),2):
			for y in range(7,-1,-1):
				output += f'{(int(string[x:x+2],16) >> y & 1)}'
		return output
	...
	def DES3MAC(self,message,key,ssc):
		...
		for y in range(len(message) / 8):
			...
			for x in range(len(mac)):
				left += f'{ord(mac[x]):02x}'
				right += f'{ord(current[x]):02x}'
			machex= f'{xor(int(left,16),int(right,16)):016x}'
	...
	def SSCIncrement(self,ssc):
		...
		return self.ToBinary(f'{out:016x}')
	...
	def TRANSITID(self,data):
		...
		if(lrc != int(data[48:56],2)):
			print('LRC mismatch: {int(data[48:56],2):02X} should be {lrc:02X}!')
			return 0
		out= f'{int(data[16:48],2):08X}'
		return out
	...
	def FDXBIDEncode(self,appid,ccode,natid):
		"Encode FDX-B ID"
		hexccode= f'{(int(ccode,10) << 2):03x}'
		glue = int(hexccode[-1:],16) & 0xc
		hexccode = hexccode[:-1]
		hexid= f'{int(natid,10):010x}'
	...
	def FDXBIDPrint(self,data):
		...
		print(self.FDXBCCODE, end=' ')
		print(f"({self.FDXBCCODEHR})")
	...
	def PCSCPrintATR(self,data):
		...
		print(f'            {data[3]}  {histlen} bytes historical data follow')
		if data[4] == '8':
		...
		if data[8:12] == self.PCSC_CSC:
			...
			print(f'                     {data[12:14]}  {applen} bytes follow') 
			print(f'                 RID:  {data[14:24]} ', end=' ')
			if data[14:24].upper() == self.PCSC_RID:
				print('PC/SC Workgroup')
			else:
				print('Unknown RID')
			pixlen= applen - 5
			print(f'                           PIX:  {data[24:24 + pixlen * 2]}')
			ss= data[24:26]
			print(f'                            SS:  {ss} {self.PCSC_SS[ss]} ', end=' ')
			# if card is ISO15693 print manufacturer name
			if 9 <= int(ss,16) <= 12:
				try:
					print(f'({self.ISO7816Manufacturer[self.uid[2:4]]})')
				except:
					print('(Unknown Manufacturer)')
			else:
				print()
			print(f'                            Name:  {data[26:30]}  {self.PCSC_NAME[data[26:30]]}')
			print(f'                                 RFU:  {data[30:-2]}')
			spaces= histlen * 2
		else:
			print('                 Detected SMARTCARD')
			print('            ATS:',self.pcsc_ats,'-', end=' ')
			...
			if atsbyte == '00' or atsbyte == '10' or self.pcsc_ats[0] == '8':
				print(f'       Category: {atsbyte}  Format according to ISO/IEC 7816-4')
			else:
				print(f'       Category: {atsbyte}  Proprietary format')
			spaces= len(self.pcsc_ats)
		space= ''
		for x in range(spaces):
			space += ' '
			
		print(f'{space}    Checksum TCK: {data[-2:]}', end=' ')
		...
		if int(data[-2:],16) == tck:
			...
			print(f'(Checksum error: {tck:02x})')
			return False
