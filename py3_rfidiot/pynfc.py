...
class ISO14443A(object):
	...
	def __str__(self):
		rv = f"ISO14443A(uid='{self.uid}', atr='{self.atr}', atqa='{self.atqa}', sak='{self.sak}')"
		return rv
...
class ISO14443B(object):
	def __init__(self, ti):
		...
		self.cid= f"{ti.ui8CardIdentifier:02x}"
		self.atr = ""        # idem
...
class JEWEL(object):
	...
	def __str__(self):
		rv = f"JEWEL(btSensRes='{self.btSensRes}', btId='{self.btId}')"
		return rv
...
#	def initLog(self, level=logging.INFO):
	...
	def listreaders(self, target):
		...
		print(f'LibNFC ver {self.libnfc.nfc_version()} devices ({nfc_num_devices}):')
		if nfc_num_devices == 0:
			print('\t', 'no supported devices!')
			return
		for i in range(nfc_num_devices):
			if devices[i]:
				self.log.debug(f"nfc_open: {ctypes.cast(devices[i].connstring,ctypes.c_char_p).value}")
	...
	def configure(self, nfcreader):
		if rfidiotglobals.Debug:
			...
			self.log.debug(f"Connecting to NFC reader number: {repr(nfcreader)}") # nfcreader may be none
		if nfcreader != None:
		...
		if rfidiotglobals.Debug:
			if self.device == None:
				...
				self.log.debug("Opened NFC reader LIBNFC READER")#self.LIBNFC_READER.decode())	
	...
	def sendAPDU(self, apdu, timeout=None):
		...
		if rfidiotglobals.Debug:	
			self.log.debug(f"Sending {len(tx)} byte APDU:", "".join([f"{x:02X}" for x in tx]))
